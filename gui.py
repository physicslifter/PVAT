...
class AnalysisGUI:
    def __init__(self):
        ...
        self.real_data_csv = "data/real_info.csv"
    ...
    def prompt_open_saved(self):
        ...
        left_ax = plt.axes([0.35, 0.8, 0.25, 0.1])
        left_ax.set_title("Select Analysis Folder          OR          Search Your Analysis")
        left_ax.axis('off')
        ...
        for i, folder in enumerate(folders):
            ax_btn = plt.axes([0.07, y0 - i*button_height, 0.3, button_height*0.9])
            btn = Button(ax_btn, folder)
            btn.on_clicked(lambda event, f=folder: self.list_folder_files(f))
        ...
        type_radio = RadioButtons(ax_type, ['BeamRef', 'Shot', 'ShotRef', 'Other'])
    ...
    def list_folder_files(self, folder):
        ...
        # Example: list subfolders/files by type
    ...
    def list_search_results(self):
        info_csv = os.path.join(self.base_analysis_dir, "info.csv")
        if not os.path.exists(info_csv):
            self.ax.set_title("No info.csv found in Analysis folder.")
            plt.draw()
            return
        df = pd.read_csv(info_csv, dtype=str)
    ...
    def prompt_real_data_inputs(self):
        ...
        type_radio = RadioButtons(type_ax, ['beam_ref', 'shot', 'shot_ref'])
        btn_submit = Button(plt.axes([0.4, 0.2, 0.25, 0.08]), 'Search')

        self.real_input = {'shot': '', 'visar': '', 'type': 'beam_ref'}
    ...
    def search_real_data_files(self):
        ...
        files = filtered['filename'].dropna().tolist() if 'filename' in filtered.columns else []
        ...
        if not files:
                ...
                def on_click(event):
                    ...
                    for btn in self.widgets:
                        if hasattr(btn, "color"):
                            btn.color = '0.85'
                    event.inaxes.set_facecolor('lightblue')
                    plt.draw()
                return on_click
            ...
            for i, (f, t) in enumerate(zip(files, types)):
                ...
                btn.on_clicked(make_on_click(f, t))
                self.widgets.append(btn)
    ...
    def launch_analysis(self, filename, filetype):
            self.clear_widgets()
            self.ax.text(0.5, 0.5, f"Launching {filetype} analysis for\n{filename}", fontsize=14, ha='center', va='center')
            plt.draw()
            # Call your BeamAligner, ShotAligner, etc.
    ...
    def launch_synthetic_analysis(self, analysis_type):
        ...
        if not files:
            ...
            plt.draw()
            return
        self.ax.set_title(f"Files found: {', '.join(files)}")
        plt.draw()
