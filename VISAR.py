...
class JLFVISAR:
    def __init__(self, 
                 ...
                 cameraID:int=820019):
        """
        - Shot no is the # of the shot in the Analysis folder
        - sweep speed is the sweep time of the camera in ns
        - VISAR is which instrument you want to look at for the shot (1 or 2)
        - slit size is the distance of the slit opening in um
        - camera ID is the ID of the hamamatsu
        """
        self.sweep_speed = sweep_speed
        self.slit_size = slit_size
        self.cameraID = cameraID
        if VISAR not in [1,2]:
            raise Exception("VISAR instrument must be 1 or 2")
        self.shot_no = shot_no
        try:
            self.shot_files = [
                f"Data/V{VISAR}/{shot_no}.tif",
                f"Data/V{VISAR}/{shot_no}_ref.tif"
            ]
        except:
            raise Exception(f"Data not found for shot {shot_no}")
        self.space_aligned = False
        self.has_data = False
    ...
    def get_data(self):
        for file in self.shot_files:
            tif = Image.open(file)
            if "ref" in file:
                self.ref_data = np.array(tif).T
            else:
                self.data = np.array(tif).T

    def align_time(self):
        #look to see if there is a timing calibration
        if os.path.exists(f"Analysis/{self.shot_no}/timing_calibration.csv"):
            data = pd.read_csv(f"Analysis/{self.shot_no}/timing_calibration.csv")
            self.time = data.at_time
        else:
            print(self.sweep_speed)
    ...
    def take_lineout(self, min, max):
        """
        takes lineout between min and max values
        """
    ...
    def show_data(self, ax, minmax=(300, 4000), ref:bool=False):
        ...
        if self.space_aligned == False:
            self.align_space()
        data = self.data if ref == False else self.ref_data
        vmin, vmax = minmax
        self.vmin = vmin
        self.vmax = vmax
